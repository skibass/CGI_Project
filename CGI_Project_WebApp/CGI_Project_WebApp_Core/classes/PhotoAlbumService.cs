...
namespace CGI_Project_WebApp_Core.classes
{
    public class PhotoAlbumService
    {
        ...
        public async Task TryAddUserPicture(Employee employee, string path, IFormFile Upload)
        {
            string Guidstring = Guid.NewGuid().ToString();
            string UploadName = Path.Combine(path, Guidstring + Upload.FileName);
            ...
            employee.ProfileImage = Guidstring + Upload.FileName;
            await AddUserPicture(employee);

        }
        private async Task AddUserPicture(Employee employee)
        {
            _photoAlbumRepository.AddUserPicture(employee);
        }
